...
### 测试
- **大幅提升测试覆盖率**
  - 新增 api_health_history、api_pricing、proxy 模块的全面测试
  - 修复所有失败的 proxy 和 store 测试
