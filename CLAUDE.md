...
- 重试与超时：`PROXY_RETRY_MAX/RETRY_MAX_ATTEMPTS=5`；单次超时 20/14/8/4/2s，`RETRY_TOTAL_TIMEOUT_SEC=60`，`RETRY_ON_STATUS=408,429,500,502,503,504,522,524`
- 健康检查：默认 `PROXY_HEALTH_CHECK_MODE=head`；`PROXY_FAIL_THRESHOLD=6`，`PROXY_HEALTH_INTERVAL_SEC=60`，`HEALTH_ALL_INTERVAL_MIN=20`，并发 `HEALTH_CHECK_CONCURRENCY=1`
- 熔断：窗口 240s，失败率 0.95，连续失败 20 次，冷却 120s，半开探测 12 次
- 预热：`WARMUP_ATTEMPTS=2`，`WARMUP_TIMEOUT_MS=12000`
- 传输层超时：TLS 握手 20s，响应头 60s
